import { useCart } from "../context/Cartcontext";
import AddButton from "./AddButton";

function Clickimg({selectedImage,setSelectedItemId,isVeg,price, description ,name ,rating, info}) {
  const { addToCart, getItemQuantity, increaseQuantity, decreaseQuantity, removeItem } = useCart();

  const quantity = info ? getItemQuantity(info.id) : 0;


return(
    <>
    {selectedImage && (
        <div className="fixed inset-0 flex justify-center items-center bg-black/70 backdrop-blur-s z-50" onClick={() => setSelectedItemId(null)}>
        <div >
        <p className="text-end relative top-5 right-5 cursor-pointer h-0 " onClick={()=>removeItem(item)}>✖</p>
          <img src={selectedImage} alt="Big view" className="w-120 h-70 object-cover rounded-t-4xl" />
          
            <div className="flex bg-white justify-between w-120">
          <div className="pl-5 pt-5">
          <div className= {isVeg ? 'text-green-600' : 'text-red-600'}>
                        <p className="border-3 w-5">
                          <div className="m-0.5 border-5 rounded-2xl"></div>
                        </p>
                      </div>
                      <p className="text-lg font-bold text-stone-700">{name}</p>
                      <p className="font-semibold">₹{price}</p>
                      <span className="flex"><img
                          className="w-4 mt-1 mr-1 h-4 rounded-4xl bg-green-700"
                          src="data:image/png;base64,"
                          alt=""
                        />{rating}</span>
                        </div>
                   <div className="pr-5 pt-9" onClick={(e) => e.stopPropagation()}> <AddButton
                  info={info}
                  quantity={quantity}
                  addToCart={addToCart}
                  increaseQuantity={increaseQuantity}
                  decreaseQuantity={decreaseQuantity}
                  
                /></div>
                   </div>
                   <p className="w-120 bg-white p-5 text-gray-700 rounded-b-4xl ">{description}</p>
        </div></div>
      )}
    </>
)
}
export default Clickimg;