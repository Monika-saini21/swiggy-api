import { Link } from 'react-router-dom';
import { useFilters } from '../context/FilterContext';
import useCards from '../hooks/useCards';
import Orderdetail from '../components/Orderdetail.jsx'
import Offline from './Offline';


function Cards() {
   const { filters } = useFilters();
   const data = useCards();
   const allRestaurants = data?.data?.cards[1]?.card?.card?.gridElements?.infoWithStyle?.restaurants || [];
  
 
  const filteredRestaurants = allRestaurants.filter((res) => {
    const info = res.info;
    if (filters.includes('Fast Delivery') && info.sla?.deliveryTime > 25) return false;
    if (filters.includes('Ratings 4.5+') && info.avgRating < 4.5) return false;
    if (filters.includes('Pure Veg') && !info.veg) return false;
   
    if (filters.includes('Offers') && !info.aggregatedDiscountInfoV3) return false;
   
    return true;
  });

 

  return (
    <>
    <Orderdetail />
    <div className="flex justify-center  ">
    <Offline/>
    </div>
     
    <div className='flex justify-center flex-wrap w-50% gap-9 m-18 mt-4'>
      {filteredRestaurants.map((data) => (
        <Link key={data.info.id} to={`restaurants/${data.info.id}`}>
       <div className=' mb-3 w-60 hover:scale-95 duration-300'>
       
        <div className="relative w-64 h-36 overflow-hidden rounded-lg">
      <img  className='w-60  h-36 object-cover rounded-2xl  ' src={`https://media-assets.swiggy.com/swiggy/image/upload/${data.info.cloudinaryImageId}`} alt="" srcset="" />
    <div className="absolute bottom-0 left-0 w-60 h-20 rounded-2xl bg-gradient-to-t from-black/90 to-transparent"></div></div>
    {data.info.aggregatedDiscountInfoV3?.subHeader && (
          <p className='relative bottom-6 left-3 font-bold h-0 text-white'>{data.info.aggregatedDiscountInfoV3.subHeader}</p>
        )}
      <h2 className='font-bold truncate ml-3 '>{data.info.name}</h2>
     
      <samp  className='font-bold flex font ml-3 '><img className='w-4 mt-1 mr-1 h-4 rounded-4xl bg-green-700' src="data:image/png;base64," alt=""  />{data.info.avgRating}<samp className='relative bottom-1  font-bold'>.</samp>{data.info.sla.deliveryTime} mins </samp>
      <p className='truncate  text-sm text-gray-500 ml-3'>{data.info.cuisines.join(', ')}</p>
      <p className='text-sm  text-gray-500 ml-3'>{data.info.areaName}</p>
    </div>
        </Link>
      ))}
     
    </div>
 
   
    </>
  );
}
export default Cards;


