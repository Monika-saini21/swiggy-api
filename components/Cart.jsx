          ...
          {cartItems.map((item) => {
            ...
            <li className="flex mt-7  justify-between w-173 items-start border-2 border-orange-400 shadow-2xl p-3 rounded-2xl" key={item.id}>
              <div >
                <samp className='flex'>
                <div
                    className={isVeg ? "text-green-600" : "text-red-600"}><p className='border-3 w-4.5'><div className='m-0.5 border-5 rounded-2xl'></div></p>
                    </div>
              <p className='text-sm ml-3 truncate font-semibold'>{item.name}</p>
              </samp>
              <p className='text-sm py-2'>₹{item.price / 100 || item.defaultPrice / 100}</p>
